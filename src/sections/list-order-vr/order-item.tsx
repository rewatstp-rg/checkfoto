...
import { Divider, Typography, IconButton, ListItemText } from '@mui/material';
...
export default function OrderItem({ order, handleDeleteOrder }: Props) {
  ...
  const { orderNumber, eventType, eventNameTh, fullName, eventNameEn, imageEventBannerUrl, netAmount, status, paymentMethodDesc, paymentMethodDescEn, statusDesc, statusDescEn, eventDate, orderDate, paymentExpiredDate } = order;

  const linkTo = paths.order.details(orderNumber || '');
  const lintToOrderVr = paths.orderVirtualRun.details(orderNumber || '');
      ...
      <Stack direction="row" alignItems="center" justifyContent="space-between" gap={2}>
...

        {
          (eventType === VIRTUAL_RUN && status === 'PAID') ? <LoadingButton
            ...
            onClick={() => router.push(lintToOrderVr)}
          >
            {t('orders.dashborad')}
          </LoadingButton> : null
        }
